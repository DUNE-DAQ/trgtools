...
{
  // No-op, but available for expansions or finalize operations if needed
}
...
{
  TH1D* h = new TH1D("h",
                     Form("h ch%i SlotID: %i- Run: %i, Trigger %i",
                     tp.header.channel, tp.daq_header.slot_id, run, trgnum),
                     1024, 0, 1024);

  for(int j = 0; j < 1024; j++)
    h->SetBinContent(j+1, tp.get_adc(j));
  ...
  TLine* baseline = new TLine(0, tp.header.get_baseline(), 1000, tp.header.get_baseline());
  ...
  {
    tl1.push_back(new TLine(64+tp.get_time_peak(i),
                            tp.header.get_baseline(),
                            64+tp.get_time_peak(i),
                            tp.header.get_baseline()-tp.get_max_peak(i)));
    tl2.push_back(new TLine(tp.get_time_peak(i),
                            h->GetMinimum(),
                            tp.get_time_peak(i),
                            h->GetMaximum()));

    tl3.push_back(new TLine(64+tp.get_time_peak(0),
                            tp.header.get_baseline(),
                            64+tp.get_time_peak(0)+tp.get_time_pulse(0),
                            tp.header.get_baseline()));

    tl4.push_back(new TLine(64+tp.get_time_peak(0)+tp.get_time_pulse(0),
                            tp.header.get_baseline(),
                            64+tp.get_time_peak(0)+tp.get_time_pulse(0)+tp.get_time_pulse_ob(0),
                            tp.header.get_baseline()));
  ...
  long long int q = 0;
  for(int i = tp.get_time_peak(0);
      i < tp.get_time_peak(0) + tp.get_time_pulse(0);
      i++)
  {
    q += h->GetBinContent(i+1) - tp.header.get_baseline();
  }
  ...
  {
    pt[i] = new TPaveText(titwidth+(1.0-titwidth)/5*(i),
                          0.0,
                          titwidth+(1.0-titwidth)/5*(i+1),
                          1.0);
...
int main(int argc, char const *argv[])
{
  ...
  rp.set_processor([&](daqdataformats::TimeSlice& tsl) -> void {
    const auto& frags = tsl.get_fragments_ref();
    ...
    for (size_t i = 0; i < num_frags; i++) {
      ...
      fddetdataformats::DAPHNEFrame* tp_array =
        static_cast<fddetdataformats::DAPHNEFrame*>(frag->get_data());

      std::map<int,int> ch;
      for(size_t i_tp = 0; i_tp < n_tps; ++i_tp) {
        auto& tp = tp_array[i_tp];
        ch[tp.get_channel()] = 1;

        if (!quiet) {
          std::cout << tp.daq_header << std::endl;
          fmt::print("  ch: {}  Baseline: {}, DAs ({},{},{},{},{})\n",
                     tp.get_channel(),
                     tp.header.get_baseline(),
                     tp.get_da(0), tp.get_da(1),
                     tp.get_da(2), tp.get_da(3), tp.get_da(4));
        }

        for(size_t j=0; j<5; j++) {
          if(tp.get_da(j)==1) {
            auto thistp = tp.get_TP(j);
        ...
        if(num_wav > 0) {
          if(pdfcounter == 0) {
            Print(tp, frag->get_header().run_number, frag->get_header().trigger_number, 1);
            pdfcounter++;
          } else if(num_wav > pdfcounter) {
            Print(tp, frag->get_header().run_number, frag->get_header().trigger_number, 0);
            pdfcounter++;
          } else if(num_wav == pdfcounter) {
            Print(tp, frag->get_header().run_number, frag->get_header().trigger_number, -1);
            pdfcounter++;
      ...
      std::sort(tp_buffer.begin(), tp_buffer.end(),
                [](auto& a, auto& b) {
                  return a.get_time_peak() < b.get_time_peak();
                });
      ...
      // TA Processing
      //
      auto ta_start = std::chrono::steady_clock::now();
      std::unique_ptr<daqdataformats::Fragment> ta_frag = ta_emulator->emulate_vector(tp_buffer);
      auto ta_end = std::chrono::steady_clock::now();
      uint64_t ta_diff = std::chrono::nanoseconds(ta_end - ta_start).count();
      ...
      double t0 = 0;
      if (!tp_buffer.empty()) {
        t0 = tp_buffer[0].get_time_peak();
        if(myAbsolutt0 == 0)
          myAbsolutt0 = tp_buffer[0].get_time_start();
      }

      for(auto &thistp : tp_buffer) {
        ntp_tp.Fill(frag->get_header().trigger_number,
                    thistp.get_channel(),
                    0,
                    static_cast<float>(thistp.get_adc_peak()),
                    thistp.get_adc_integral(),
                    thistp.get_time_peak() - myAbsolutt0,
                    thistp.get_time_start() - myAbsolutt0,
                    thistp.get_time_over_threshold());
      }
      ...
      if(!ta_list.empty() && ta_list[0].time_peak < t0) {
        // Remove first TA if it corresponds to a previous event
        ta_list.erase(ta_list.begin());
      }

      int ii = 0;
      double myt0 = 0;
      if(!ta_list.empty())
        myt0 = ta_list[0].time_start;

      for(auto &ta_bit : ta_list) {
        std::map<int,int> channelcounter;
        if(!quiet)
          std::cout << " TA " << ii << " " << ta_bit.inputs.size()
                    << " " << ta_bit.adc_integral
                    << "  " << ta_bit.time_start - myt0
                    << " " << ta_bit.time_end - myt0
                    << " TimePeak: " << ta_bit.time_peak
                    << std::endl;

        for(auto &mytp : ta_bit.inputs) {
          channelcounter[mytp.channel]++;
        }
        ntp_ta.Fill(frag->get_header().trigger_number,
                    channelcounter.size(),
                    ta_bit.inputs.size(),
                    ta_bit.adc_peak,
                    ta_bit.adc_integral,
                    ta_bit.time_start - myAbsolutt0,
                    ta_bit.time_peak - myAbsolutt0,
                    ta_bit.time_end - myAbsolutt0,
                    ta_bit.time_end - ta_bit.time_start);
        ii++;
      }
      ntp_ev.Fill(frag->get_header().trigger_number, tp_buffer.size(), ta_list.size());

      // Adjust fragment header for TA
      daqdataformats::FragmentHeader frag_hdr = frag->get_header();
      frag_hdr.element_id = daqdataformats::SourceID{
        daqdataformats::SourceID::Subsystem::kTrigger,
        frag->get_element_id().id + 1000
      };
      ta_frag->set_header_fields(frag_hdr);
      ta_frag->set_type(daqdataformats::FragmentType::kTriggerActivityPDS);
      tsl.add_fragment(std::move(ta_frag));

      //
      // TC Processing
      //
      auto ta_buffer = ta_emulator->get_last_output_buffer();
      ...
      uint64_t tc_diff = std::chrono::nanoseconds(tc_end - tc_start).count();
