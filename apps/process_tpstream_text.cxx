...
int main(int argc, char const *argv[])
{
  ...
  uint64_t num_rec(0);
  app.add_option("-n", num_rec, "Num records");

  // AAA: temporary vars for development
  uint64_t eps_selection(3);
  app.add_option("-e", eps_selection, "eps: maximum distance two points can be from one another while still belonging to the same cluster");
  uint64_t min_pts_selection(10);
  app.add_option("-p", min_pts_selection, "Minimum number of points for a cluster");
  ...
  triggeralgs::TriggerActivityMakerDBSCAN trigger_activity_algo;
  ...
  */
  const nlohmann::json config = {
      {"eps", eps_selection},
      {"min_pts", min_pts_selection}
  };

  trigger_activity_algo.configure(config);
  ...
  rp.set_processor([&]( daqdataformats::TimeSlice& tsl ) -> void {
    ...
    for( const auto& frag : frags ) {
      ...
      for( const auto& tp : tp_buffer ) {
        ...
        trigger_activity_algo(tp, ta_buffer);
